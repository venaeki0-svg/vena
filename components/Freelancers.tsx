...
import { TeamMember, TeamProjectPayment, Profile, Transaction, TransactionType, TeamPaymentRecord, Project, RewardLedgerEntry, Card, FinancialPocket, PocketType, PerformanceNoteType, PerformanceNote, NavigationAction, QRCodeRecord } from '../types';
...
export const Freelancers: React.FC<FreelancersProps> = ({
    ...
    rewardLedgerEntries, setRewardLedgerEntries, pockets, setPockets, cards, setCards, qrCodes
}) => {
    ...
    useEffect(() => {
        if (paymentSlipToView && showDigitalSignature) {
            const qrCodeContainer = document.getElementById(`payment-slip-qrcode-${paymentSlipToView.id}`);
            if (qrCodeContainer && typeof (window as any).QRCode !== 'undefined') {
                 const generateQrCode = async () => {
                    ...
                    const dataToSign = {
                        ...
                        jumlah: formatCurrency(paymentSlipToView.totalAmount),
                        tanggal_bayar: new Date(paymentSlipToView.date).toISOString(),
                        nomor_slip: paymentSlipToView.recordNumber,
                    };
    ...
    }, [paymentSlipToView, showDigitalSignature, selectedMember, qrCodes]);
            ...
            <div id={`payment-slip-content-${record.id}`} className="printable-content bg-white text-black p-6 font-sans">
                ...
                <footer className="mt-12 text-xs text-slate-500">
                    <div className="flex justify-between items-end">
                        ...
                        <div className={`text-center ${showDigitalSignature ? '' : 'hidden'}`}>
                            <p>Diverifikasi oleh,</p>
                            <div id={`payment-slip-qrcode-${record.id}`} className="flex justify-center my-2"></div>
                            {showDigitalSignature && qrCodes[0] && typeof qrCodes[0].content === 'string' && <p className="text-[8px] text-gray-500 max-w-[100px] text-center mx-auto">{qrCodes[0].content}</p>}
                            <p className="font-medium text-slate-900 mt-1">({userProfile.authorizedSigner || userProfile.companyName})</p>
                        </div>
                    </div>
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td className="px-4 py-3 text-center">
                                                           <div className="flex items-center justify-center gap-1">
                                                                ...
                                                                <button onClick={() => { setShowDigitalSignature(false); setPaymentSlipToView(record); }} className="p-2 text-brand-text-secondary hover:bg-brand-input rounded-full" title="Lihat Slip Pembayaran"><FileTextIcon className="w-5 h-5" /></button>
                                                            </div>
                                                        </td>
                                                    </tr>
